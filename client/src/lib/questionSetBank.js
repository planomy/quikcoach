/** Launch library of teaching question sets. Bank sets are not deletable.
 *  Source audit revision: question-sets (1).json (2026-09-07, 93 sets).
 */

function q(prompts) {
  return prompts.map((item, index) => {
    if (typeof item === 'string') {
      return {
        id: `p${index + 1}`,
        type: 'short',
        prompt: item,
        options: [],
        correctAnswer: '',
      };
    }
    const prompt = String(item?.prompt || '').trim();
    const helper = String(item?.helper || item?.descriptor || '').trim().slice(0, 240);
    return {
      id: `p${index + 1}`,
      type: 'short',
      prompt,
      options: [],
      correctAnswer: '',
      ...(helper ? { helper } : {}),
    };
  }).filter((question) => question.prompt);
}

function set(def) {
  return {
    ...def,
    bank: true,
    questions: q(def.prompts),
  };
}

export const SUBJECTS = ['English', 'HASS', 'Maths', 'Science', 'General'];
export const YEAR_BANDS = ['5–6', '7–8', '9–10', '11–12'];

export const QUESTION_SET_BANK = [
  // English — curated writing library + literature / poetry / persuasive / essay
  set({
    id: 'bank-eng-01',
    name: 'Build a Strong Paragraph',
    subject: 'English',
    years: '5–6',
    skill: 'Writing',
    minutes: 10,
    prompts: [
      'What is the one main idea you need to get across?',
      'Write a first sentence that puts that idea on the table.',
      'Add one example, fact or detail that proves it.',
      'Explain how that example supports your idea.',
      'Write a last sentence that closes the paragraph.',
      'Reread. Which sentence would confuse a classmate? Quote it.',
    ],
  }),
  set({
    id: 'bank-eng-02',
    name: 'Build a Narrative Paragraph',
    subject: 'English',
    years: '5–6',
    skill: 'Creative writing',
    minutes: 10,
    prompts: [
      'Where is your character right now?',
      'What is happening in this exact moment?',
      'Add one thing they can see, hear or feel.',
      'What does your character do next?',
      'What happens because of that action?',
      'Reread. Pick your weakest detail and rewrite it here.',
    ],
  }),
  set({
    id: 'bank-eng-cw-01',
    name: 'Show, Don\'t Tell',
    subject: 'English',
    years: '7–10',
    skill: 'Creative writing',
    minutes: 10,
    note: 'Students need writing on the page. For a cold start use Get Writing — Narrative first. Best as revise — needs a draft that names an emotion.',
    prompts: [
      'Find a sentence that tells the reader how the character feels. Paste it here.',
      'Paste that sentence again with the emotion word removed.',
      'What could the character do physically instead?',
      'Add one small body-language or sensory detail. Write the new sentence.',
      'What would a reader now guess the character is feeling?',
    ],
  }),
  set({
    id: 'bank-eng-cw-02',
    name: 'Character Through Action',
    subject: 'English',
    years: '7–10',
    skill: 'Creative writing',
    minutes: 10,
    prompts: [
      'What does your character want right now?',
      'What action could reveal that want?',
      'Add one gesture, habit or choice that reveals character. Write it as a sentence.',
      'Delete one sentence that explains the character. Paste what is left.',
      'What should the reader work out for themselves?',
    ],
  }),
  set({
    id: 'bank-eng-cw-03',
    name: 'Dialogue with Purpose',
    subject: 'English',
    years: '7–10',
    skill: 'Creative writing',
    minutes: 10,
    prompts: [
      'What does each character want from this conversation?',
      'Cut one greeting, filler line or unnecessary explanation. Paste the line you cut.',
      'What is one thing the character will not say directly?',
      'Add an action or reaction that hints at what is unsaid. Write it here.',
      'Quote one line that only Character A would ever say.',
    ],
  }),
  set({
    id: 'bank-eng-cw-04',
    name: 'Build Tension',
    subject: 'English',
    years: '7–10',
    skill: 'Creative writing',
    minutes: 12,
    prompts: [
      'What does the character want?',
      'What is stopping them?',
      'Where could you delay an action or a reveal?',
      'What could you keep from the reader for now?',
      'Add one small signal that something is wrong. Write the sentence.',
      'Write your final line. Leave the pressure unresolved.',
    ],
  }),
  set({
    id: 'bank-eng-cw-05',
    name: 'Control the Pace',
    subject: 'English',
    years: '7–10',
    skill: 'Creative writing',
    minutes: 10,
    prompts: [
      'Which moment in your story deserves the most attention?',
      'Slow that moment down using action, sensory detail or thought. Write the slowed version.',
      'Which part could be shortened or skipped entirely?',
      'Write one short sentence that would land hard.',
      'Where would a paragraph break change how it reads?',
    ],
  }),
  set({
    id: 'bank-eng-cw-06',
    name: 'Make the Setting Work',
    subject: 'English',
    years: '7–10',
    skill: 'Creative writing',
    minutes: 10,
    prompts: [
      'What should the setting make the reader feel?',
      'Take one general description and rewrite it with a precise detail.',
      'Add sound, texture, smell or temperature. Write the sentence.',
      'How can the character interact with the setting?',
      'Which single detail best reinforces the mood or tension?',
    ],
  }),
  set({
    id: 'bank-eng-cw-07',
    name: 'Sensory Detail with Purpose',
    subject: 'English',
    years: '7–10',
    skill: 'Creative writing',
    minutes: 10,
    prompts: [
      'Which sense matters most in this moment?',
      'What precise detail would the character actually notice?',
      'How does this detail affect the mood or meaning?',
      'Which sensory detail adds nothing? Name it and cut it.',
      'Take one detail you kept and rewrite it so it does more work.',
    ],
  }),
  set({
    id: 'bank-eng-cw-08',
    name: 'Strong Openings',
    subject: 'English',
    years: '7–10',
    skill: 'Creative writing',
    minutes: 10,
    prompts: [
      'What is the first thing that actually happens in your story?',
      'Could you start there instead of earlier? What would you cut?',
      'Which background information can wait until later?',
      'What question or tension should your opening create?',
      'Write a first sentence that makes the reader need the second one.',
    ],
  }),
  set({
    id: 'bank-eng-cw-09',
    name: 'Strong Endings',
    subject: 'English',
    years: '7–10',
    skill: 'Creative writing',
    minutes: 10,
    prompts: [
      'What has changed by the end of your story?',
      'Which of your final sentences explains too much? Paste it.',
      'Could an image, action or line of dialogue carry that meaning instead? Write it.',
      'What should the reader feel or wonder as they finish?',
      'Paste your ending with the explanation cut. Which version is stronger, and why?',
    ],
  }),
  set({
    id: 'bank-eng-cw-10',
    name: 'Sentence Craft & Precision',
    subject: 'English',
    years: '7–10',
    skill: 'Creative writing',
    minutes: 10,
    note: 'Line editing. Fire this after a draft exists, not before. Best as revise — needs a draft students can edit line by line.',
    prompts: [
      'Find a weak verb and replace it. Type: old verb → new verb.',
      'Find a vague noun or description and rewrite it so it is specific.',
      'Find a predictable sentence opening and rewrite the opening.',
      'Find an overloaded sentence. Split it and paste both halves.',
      'Which words add length but not meaning? Quote them, then cut them.',
      'Read it aloud. Paste the sentence where the rhythm still feels wrong.',
    ],
  }),
  set({
    id: 'bank-eng-cw-11',
    name: 'Narrative Voice & Point of View',
    subject: 'English',
    years: '9–12',
    skill: 'Creative writing',
    minutes: 12,
    prompts: [
      'Who is telling this moment?',
      'What can this narrator know, and what can they not know?',
      'What would this character notice first?',
      'Quote one phrase of yours that sounds like you rather than the narrator.',
      'Find one sentence that breaks the point of view. Paste it.',
      'Rewrite that sentence through the narrator\'s perspective.',
    ],
  }),
  set({
    id: 'bank-eng-cw-12',
    name: 'Narrative Essay Planner',
    subject: 'English',
    years: '7–10',
    skill: 'Creative writing',
    minutes: 12,
    note: 'Whole-story planner before drafting. Send to Inbox as a keepable prompt set, or to Respond to collect plans.',
    prompts: [
      'What is the task?',
      'How many words?',
      'When is it due?',
      {
        prompt: 'What is your pitch (who, where, what, problem)?',
        helper: 'One tight sentence a classmate could remember.',
      },
      'How will it resolve?',
      {
        prompt: 'List the paragraphs you will use (eg: hook, who, where, what, flashback, tension, complication, climax, resolution).',
        helper: 'Choose only the beats you need — not every label.',
      },
      'What will change for your character by the end of the story?',
      {
        prompt: 'What specific theme, feature or motif do you need to include?',
        helper: 'Name the requirement from the task so you do not forget it mid-draft.',
      },
    ],
  }),
  set({
    id: 'bank-eng-an-01',
    name: 'Stop Retelling the Plot',
    subject: 'English',
    years: '7–10',
    skill: 'Analytical writing',
    minutes: 10,
    note: 'Best as revise — needs a draft paragraph.',
    prompts: [
      'Paste the sentence that simply tells me what happened.',
      'What does this event reveal?',
      'Why has the author included it?',
      'What idea does it develop?',
      'Rewrite the point so it makes an argument rather than retelling.',
    ],
  }),
  set({
    id: 'bank-eng-an-02',
    name: 'Make a Real Analytical Claim',
    subject: 'English',
    years: '7–10',
    skill: 'Analytical writing',
    minutes: 10,
    prompts: [
      'What are you actually arguing?',
      'What would someone who disagreed with you say?',
      'What does the text suggest, rather than simply show?',
      'Rewrite your claim so it is more specific.',
      'Rewrite it again so it directly answers the question.',
    ],
  }),
  set({
    id: 'bank-eng-an-03',
    name: 'Strong Topic Sentences',
    subject: 'English',
    years: '7–10',
    skill: 'Analytical writing',
    minutes: 10,
    prompts: [
      'What is this paragraph arguing?',
      'Which key word from the question should it address?',
      'Paste your topic sentence with any plot summary removed.',
      'Rewrite it so it interprets rather than describes.',
      'Which words in it tie back to your thesis? Quote them.',
    ],
  }),
  set({
    id: 'bank-eng-an-04',
    name: 'Choose Better Evidence',
    subject: 'English',
    years: '7–10',
    skill: 'Analytical writing',
    minutes: 10,
    note: 'Works cold or on a draft — needs a studied text.',
    prompts: [
      'Which piece of evidence best proves your point? Paste it.',
      'Now cut it down to its strongest six words.',
      'Which word or phrase in it carries the most meaning?',
      'Is there a more precise example? Paste it if so.',
      'Why is this evidence stronger than the alternative?',
    ],
  }),
  set({
    id: 'bank-eng-an-05',
    name: 'Unpack the Quote',
    subject: 'English',
    years: '7–10',
    skill: 'Analytical writing',
    minutes: 10,
    note: 'Needs a quote.',
    prompts: [
      'Which word or phrase matters most?',
      'What does it suggest?',
      'What associations does that word carry?',
      'Why might the author have chosen it?',
      'How does it support your argument?',
    ],
  }),
  set({
    id: 'bank-eng-an-06',
    name: 'Authorial Choices',
    subject: 'English',
    years: '9–12',
    skill: 'Analytical writing',
    minutes: 12,
    note: 'Needs a studied text.',
    prompts: [
      'What deliberate choice has the author made?',
      'Which aesthetic feature or literary device is being used?',
      'How does it shape meaning?',
      'Why is this choice effective here?',
      'How does it support your interpretation?',
    ],
  }),
  set({
    id: 'bank-eng-an-07',
    name: 'Audience Positioning',
    subject: 'English',
    years: '9–12',
    skill: 'Analytical writing',
    minutes: 10,
    note: 'Needs a studied text.',
    prompts: [
      'How is the audience positioned to respond?',
      'What might the audience think, feel, question or believe?',
      'Which authorial choice creates this response?',
      'Why does this response matter?',
      'How does audience positioning support the author\'s purpose?',
    ],
  }),
  set({
    id: 'bank-eng-an-08',
    name: 'Culture, Values & Beliefs',
    subject: 'English',
    years: '9–12',
    skill: 'Analytical writing',
    minutes: 12,
    note: 'Needs a studied text.',
    prompts: [
      'What assumption, attitude, value or belief is represented here?',
      'Is the text reinforcing, questioning or challenging it? Explain which.',
      'Which character, event or authorial choice reveals this?',
      'How might different audiences respond?',
      'How does this shape the text\'s wider meaning?',
    ],
  }),
  set({
    id: 'bank-eng-an-09',
    name: 'Higher Insight',
    subject: 'English',
    years: '9–12',
    skill: 'Analytical writing',
    minutes: 10,
    note: 'Needs a studied text.',
    prompts: [
      'What bigger idea does this reveal?',
      'What does this suggest about people, society, culture, identity, power, values or the wider issue?',
      'Why does this matter beyond this single moment?',
      'Name one contradiction, tension or complexity here.',
      'How does this deepen your interpretation?',
    ],
  }),
  set({
    id: 'bank-eng-an-10',
    name: 'Link Evidence to Argument',
    subject: 'English',
    years: '7–10',
    skill: 'Analytical writing',
    minutes: 10,
    note: 'Best as revise — needs a paragraph with evidence already in it.',
    prompts: [
      'How does this evidence prove your point?',
      'What part of your argument does it strengthen?',
      'What key idea does it connect to?',
      'Rewrite your final sentence so it clearly returns to the argument.',
      'Which sentence in your paragraph answers the question? Quote it.',
    ],
  }),
  set({
    id: 'bank-eng-an-11',
    name: 'Extend & Synthesise',
    subject: 'English',
    years: '9–12',
    skill: 'Analytical writing',
    minutes: 12,
    note: 'Needs a studied text.',
    prompts: [
      'What second piece of evidence could develop this idea?',
      'Does it reinforce, contrast or complicate the first example? Explain which.',
      'What new layer does it add?',
      'Write the sentence that compares the two examples.',
      'What stronger conclusion can you now draw?',
    ],
  }),
  set({
    id: 'bank-eng-teacher',
    name: 'TEACHER Analytical Paragraph',
    subject: 'English',
    years: '9–12',
    skill: 'Analytical writing',
    minutes: 15,
    note: 'Flagship exam scaffold: Topic, Evidence, Author+Audience, Culture, Higher insight, Extend, Return. Needs a studied text.',
    prompts: [
      {
        prompt: 'T — Topic / Thesis: What is your paragraph arguing?',
        helper: 'Write a clear point that answers the question.',
      },
      {
        prompt: 'E — Evidence: What evidence best proves your point?',
        helper: 'Use a precise quote, example or textual detail.',
      },
      {
        prompt: 'A — Author + Audience: What has the author done, and how does it position the audience?',
        helper: 'Analyse an aesthetic feature, literary device, language or structural choice, then explain what it makes the audience think, feel, notice or believe.',
      },
      {
        prompt: 'C — Culture: What assumptions, attitudes, values or beliefs are represented?',
        helper: 'Explain what the text reinforces, questions or challenges.',
      },
      {
        prompt: 'H — Human / Higher Insight: What bigger idea does this reveal?',
        helper: 'Think beyond the quote: people, society, culture, identity, power, values or the wider issue.',
      },
      {
        prompt: 'E — Extend: What further evidence develops or complicates your idea?',
        helper: 'Add a second example that strengthens, contrasts or deepens the analysis.',
      },
      {
        prompt: 'R — Return: How does this return to your argument?',
        helper: 'Tie the paragraph back to the question and your overall thesis.',
      },
    ],
  }),
  set({
    id: 'bank-eng-05',
    name: 'Analytical Paragraph Builder',
    subject: 'English',
    years: '7–8',
    skill: 'Text analysis',
    minutes: 12,
    note: 'Junior version of TEACHER. Overlap is deliberate. Needs a studied text.',
    prompts: [
      {
        prompt: 'What claim are you making about the text?',
        helper: 'Not what happens — what it means.',
      },
      'Choose the evidence that best proves that claim.',
      'Zoom in on one important word, phrase or technique.',
      'What does it suggest that a casual reader might miss?',
      'What does the author want the reader to think or feel?',
      'Rewrite your final sentence so it returns to your claim.',
    ],
  }),
  set({
    id: 'bank-eng-08',
    name: 'Character Analysis',
    subject: 'English',
    years: '7–8',
    skill: 'Literature',
    minutes: 10,
    note: 'Needs a studied text.',
    prompts: [
      'What do we learn about this character that is not said outright?',
      'Choose one moment that reveals this.',
      'What do they say, do, avoid or notice?',
      'Which detail carries the most weight?',
      'What does that detail suggest about them?',
      'Has our understanding of the character shifted by the end? Explain how.',
    ],
  }),
  set({
    id: 'bank-eng-09',
    name: 'Theme Analysis',
    subject: 'English',
    years: '9–10',
    skill: 'Literature',
    minutes: 12,
    note: 'Needs a studied text.',
    prompts: [
      'What important idea is the text exploring?',
      'Where is that idea under pressure or in conflict?',
      'Which character, event, image or technique develops it?',
      'How does the idea become more complex as the text unfolds?',
      'What might the author be pushing the audience to consider?',
      'How does this idea shape the text as a whole?',
    ],
  }),
  set({
    id: 'bank-eng-10',
    name: 'Poetry Close Reading',
    subject: 'English',
    years: '7–8',
    skill: 'Poetry',
    minutes: 10,
    note: 'Needs a poem on screen or in hand.',
    prompts: [
      'What seems to be happening in this poem?',
      'Which word or image sticks in your mind?',
      'Name one technique doing real work.',
      'What feeling or idea does that choice create?',
      'Why might the poet have chosen it instead of something simpler?',
      'In one sentence: what is the poem saying?',
    ],
  }),
  set({
    id: 'bank-eng-11',
    name: 'Poetry Analysis',
    subject: 'English',
    years: '9–10',
    skill: 'Poetry',
    minutes: 12,
    note: 'Reordered: the hardest question is no longer first. Needs a poem on screen or in hand.',
    prompts: [
      'Which image, pattern or structural feature is doing the heavy lifting?',
      'What technique is being used?',
      'How does that choice shape tone or meaning?',
      'What interpretation are you building?',
      'How does it position the reader?',
      'How does this feature connect to the poem\'s bigger ideas?',
    ],
  }),
  set({
    id: 'bank-eng-12',
    name: 'Build a Persuasive Paragraph',
    subject: 'English',
    years: '7–8',
    skill: 'Persuasive writing',
    minutes: 10,
    prompts: [
      'What do you want your audience to believe or do?',
      'Give your strongest reason — not your safest one.',
      'Add evidence, an example or a concrete explanation.',
      'Choose one word or phrase for deliberate persuasive effect. Name it and say why.',
      'Why should the audience care about this now?',
      'Write a final sentence that leaves no room for doubt.',
    ],
  }),
  set({
    id: 'bank-eng-13',
    name: 'Argument & Counterargument',
    subject: 'English',
    years: '9–10',
    skill: 'Persuasive writing',
    minutes: 12,
    prompts: [
      'State your main argument in one sentence.',
      'What evidence best supports it?',
      'What would a smart opponent say back?',
      'Why might that opposing view feel reasonable?',
      'How do you dismantle or limit that argument?',
      'How does addressing the other side make your case stronger?',
    ],
  }),
  set({
    id: 'bank-eng-14',
    name: 'Plan a Senior Essay',
    subject: 'English',
    years: '11–12',
    skill: 'Essay writing',
    minutes: 12,
    prompts: [
      {
        prompt: 'What is your overall contention?',
        helper: 'Make it arguable — someone could disagree.',
      },
      'What will each body paragraph argue?',
      'Which evidence will do the real work in each paragraph?',
      'How will the paragraphs build rather than repeat?',
      'What complexity, tension or alternative reading must you face?',
      'What should the reader understand by the final sentence?',
    ],
  }),
  set({
    id: 'bank-eng-15',
    name: 'Analytical Essay Planner',
    subject: 'English',
    years: '7–12',
    skill: 'Essay writing',
    minutes: 12,
    note: 'Whole-essay planner before drafting. Send to Inbox as a keepable prompt set, or to Respond to collect plans.',
    prompts: [
      'What is the task?',
      'How many words?',
      'When is it due?',
      {
        prompt: 'What is your thesis?',
        helper: 'One clear, arguable claim that answers the task.',
      },
      {
        prompt: 'What are your main arguments?',
        helper: 'List the body points that will prove the thesis.',
      },
      {
        prompt: 'What is the hook you will use at the start of the introduction?',
        helper: 'Open with something that earns attention — then move to the thesis.',
      },
      {
        prompt: 'What will be the takeaway sentence at the end of the conclusion?',
        helper: 'Leave the reader with the point that matters most — not a summary dump.',
      },
    ],
  }),
  set({
    id: 'bank-eng-unstick-01',
    name: 'Get Writing — Narrative',
    subject: 'English',
    years: '5–10',
    skill: 'Unstick',
    minutes: 3,
    note: 'NEW. For a student who has written nothing. Four prompts, one sentence out the other end. Overwrite prompt one with your own wording if you have a better opener.',
    prompts: [
      'Who is in your story? Just a name or a description.',
      'Where are they? One place. Be specific.',
      'What is the first thing they do there? One action.',
      'Now put those three together in one sentence. That is your opening.',
    ],
  }),
  set({
    id: 'bank-eng-unstick-02',
    name: 'Get Writing — Analytical',
    subject: 'English',
    years: '7–12',
    skill: 'Unstick',
    minutes: 3,
    note: 'NEW. For a student staring at a blank analytical response. Needs a studied text.',
    prompts: [
      'In your own words, what is the question asking you to argue about?',
      'Name one moment, line or character that connects to it.',
      'What do you think that moment shows? A rough answer is fine.',
      'Write that as one sentence starting: "The text shows that…"',
    ],
  }),
  set({
    id: 'bank-eng-speaking-01',
    name: 'Oral Presentation Prep',
    subject: 'English',
    years: '7–12',
    skill: 'Speaking',
    minutes: 10,
    prompts: [
      'What is your one main message? One sentence.',
      'How will you open so people look up?',
      'List your main points in order.',
      'Which point needs an example, image or demonstration?',
      'What will you cut if you run out of time?',
      'Write your closing line.',
      'What question will someone ask you afterwards?',
    ],
  }),
  set({
    id: 'bank-eng-speaking-02',
    name: 'Debate Prep',
    subject: 'English',
    years: '7–12',
    skill: 'Speaking',
    minutes: 10,
    note: 'Needs a topic assigned.',
    prompts: [
      'State your side in one sentence.',
      'What is your strongest argument?',
      'What evidence backs it?',
      'What is the other side\'s best argument?',
      'How will you answer it?',
      'What is the one line you want the audience to remember?',
    ],
  }),
  set({
    id: 'bank-eng-fnd-03',
    name: 'What Does It Show? (Junior Analysis)',
    subject: 'English',
    years: '5–6',
    skill: 'Text analysis',
    minutes: 10,
    note: 'Fills the junior analytical gap. Gets Year 5–6 past retelling. Needs a studied text.',
    prompts: [
      'What happens in this part of the text?',
      'Now the harder question: what does it show us about the character or the story?',
      'Which words in the text made you think that? Copy them.',
      'Why do you think the author put this bit in?',
      'Write one sentence starting: "This shows that…"',
    ],
  }),
  set({
    id: 'bank-eng-cw-13',
    name: 'Proofread & Fix',
    subject: 'English',
    years: '5–12',
    skill: 'Editing',
    minutes: 8,
    note: 'Error correction, not line editing. Pair with Sentence Craft for senior classes. Best as revise — needs a finished draft.',
    prompts: [
      'Read your work aloud in your head. Paste the sentence where you stumbled.',
      'Find one sentence with no full stop or the wrong one. Fix it and paste it.',
      'Find a word you are not sure you spelled right. Type it.',
      'Check your capital letters — names, places, sentence starts. What did you miss?',
      'Find one sentence that runs on too long. Split it and paste both halves.',
      'What mistake do you make most often?',
    ],
  }),
  set({
    id: 'bank-eng-an-12',
    name: 'Unseen Text Attack',
    subject: 'English',
    years: '9–12',
    skill: 'Analytical writing',
    minutes: 12,
    note: 'Exam conditions. Every other analytical set assumes a studied text. Needs an unseen text on screen or in hand.',
    prompts: [
      'First read. What is this text about, in one sentence?',
      'What kind of text is it, and who is it for?',
      'Second read. Which line or image stands out most?',
      'Name one technique doing real work there.',
      'What is the writer trying to make the reader think or feel?',
      'Write your claim: one sentence about what this text is doing.',
    ],
  }),

  // HASS — craft-level source & historical thinking
  set({
    id: 'bank-hass-15',
    name: 'Source Detective',
    subject: 'HASS',
    years: '5–6',
    skill: 'History',
    minutes: 10,
    note: 'Put one source on the board or in Inbox first. Needs one source on the board or in Inbox.',
    prompts: [
      'What kind of source is this — and how can you tell?',
      'Who made it, and roughly when?',
      'What does it show or tell us in plain words?',
      'Why might someone have made this?',
      'What can this source help us learn?',
      'What can it not tell us?',
    ],
  }),
  set({
    id: 'bank-hass-16',
    name: 'Whose View Is Missing?',
    subject: 'HASS',
    years: '5–6',
    skill: 'History / Civics',
    minutes: 10,
    prompts: [
      'Who are the people or groups involved?',
      'What does each group want or believe?',
      'Why might their views clash?',
      'What evidence shows you that?',
      'Whose voice is missing from this story?',
      'Why does that missing view matter?',
    ],
  }),
  set({
    id: 'bank-hass-17',
    name: 'IOPCAM Source Analysis',
    subject: 'HASS',
    years: '7–8',
    skill: 'History',
    minutes: 12,
    note: 'Classic IOPCAM — sharpened for live use with one source visible. Needs one source visible.',
    prompts: [
      {
        prompt: 'I — Information: What does the source actually tell or show?',
        helper: 'Stick to what is there, not what you already know.',
      },
      'O — Origin: Who created it, and when?',
      'P — Purpose: Why was it created?',
      'C — Context: What was happening at the time that shaped it?',
      'A — Audience: Who was meant to see or hear this?',
      'M — Message: What main idea is it pushing?',
    ],
  }),
  set({
    id: 'bank-hass-18',
    name: 'Reliability & Usefulness',
    subject: 'HASS',
    years: '9–10',
    skill: 'History',
    minutes: 12,
    note: 'Needs one source visible.',
    prompts: [
      'What claim or message does this source present?',
      'Who created it — and what did they gain from people believing it?',
      'What was their purpose?',
      'What might distort their perspective?',
      {
        prompt: 'What makes this source useful for your inquiry?',
        helper: 'Useful for what question, exactly?',
      },
      'What are its limitations?',
      'What other evidence would you want next?',
    ],
  }),
  set({
    id: 'bank-hass-19',
    name: 'Senior Source Evaluation',
    subject: 'HASS',
    years: '11–12',
    skill: 'History',
    minutes: 12,
    note: 'Needs one source visible.',
    prompts: [
      'What perspective does this source construct?',
      'How do origin and context shape that perspective?',
      'How does purpose decide what is included or left out?',
      'Which detail inside the source is most historically significant?',
      'What limitation most weakens its usefulness?',
      'What evidence could support — or seriously damage — this account?',
    ],
  }),
  set({
    id: 'bank-hass-20',
    name: 'Cause & Consequence',
    subject: 'HASS',
    years: '7–10',
    skill: 'History',
    minutes: 10,
    prompts: [
      'Name the event or change you are explaining.',
      'List the main causes — not just the dramatic one.',
      {
        prompt: 'Which cause was most significant?',
        helper: 'Significant means the outcome likely changes without it.',
      },
      'Prove that ranking in one sentence.',
      'What were the immediate consequences?',
      'What longer-term consequences still mattered?',
    ],
  }),
  set({
    id: 'bank-hass-21',
    name: 'Continuity & Change',
    subject: 'HASS',
    years: '7–10',
    skill: 'History',
    minutes: 10,
    prompts: [
      'What actually changed in this period?',
      'What stayed the same, even when you would expect it to change?',
      'Who experienced the greatest change — and who barely felt it?',
      'What drove the change?',
      'Was the change sudden, gradual, or so slow people barely noticed?',
      'Why does that pattern of change matter historically?',
    ],
  }),
  set({
    id: 'bank-hass-22',
    name: 'Read the Map / Data',
    subject: 'HASS',
    years: '7–10',
    skill: 'Geography',
    minutes: 10,
    note: 'Works best with a map, graph or table on screen. Needs a map, graph or table on screen.',
    prompts: [
      'What does this map, graph or table actually show?',
      'What spatial or data pattern stands out?',
      'Where is the pattern strongest — and weakest?',
      'Which data point does not fit the pattern?',
      'Which geographic factors could explain the pattern?',
      'What conclusion can you defend with this evidence alone?',
    ],
  }),

  // Maths
  set({
    id: 'bank-maths-23',
    name: 'Understand Before You Solve',
    subject: 'Maths',
    years: '5–6',
    skill: 'Problem solving',
    minutes: 8,
    note: 'Needs a problem to work on.',
    prompts: [
      'What is the problem asking you to find?',
      'List the numbers or facts you actually need.',
      'What operation or strategy might work — and why?',
      'Estimate the answer before you calculate.',
      'Solve it. Show your working here.',
      'Does your answer make sense in the story of the problem? Explain.',
    ],
  }),
  set({
    id: 'bank-maths-24',
    name: 'Problem-Solving Routine',
    subject: 'Maths',
    years: '7–8',
    skill: 'Problem solving',
    minutes: 12,
    note: 'Needs a problem to work on.',
    prompts: [
      'What is the unknown?',
      'What given information is useful?',
      'What information is a distraction?',
      {
        prompt: 'Choose a strategy and name it.',
        helper: 'Draw, table, equation, work backwards, simplify…',
      },
      'Show the working another student could follow.',
      'Check your answer a different way. Show that check.',
      'Explain the solution in one clean sentence.',
    ],
  }),
  set({
    id: 'bank-maths-25',
    name: 'Complex Problem Attack',
    subject: 'Maths',
    years: '9–10',
    skill: 'Problem solving',
    minutes: 15,
    note: 'Needs a problem to work on.',
    prompts: [
      'What quantities or variables are in play?',
      'What relationships link them?',
      'Which mathematical idea, formula or model fits?',
      'Break the problem into the smallest useful steps. List them.',
      'Solve — and justify each key move.',
      'Check your result against the original problem\'s conditions. Does it hold?',
      'What assumptions or limitations should you flag?',
    ],
  }),
  set({
    id: 'bank-maths-26',
    name: 'Mathematical Modelling',
    subject: 'Maths',
    years: '11–12',
    skill: 'Senior mathematics',
    minutes: 18,
    prompts: [
      'Define the real-world problem in one sentence.',
      'What decision or prediction are you supporting?',
      'What assumptions must you make — and which are risky?',
      'Identify the variables and relationships.',
      'Build a mathematical model that is simple enough to use.',
      'Solve the model. Paste the result.',
      'Interpret the result back in the original context.',
      'Is the answer reasonable for the real situation? Explain.',
      'Where does the model break down?',
    ],
  }),
  set({
    id: 'bank-maths-27',
    name: 'Find the Error',
    subject: 'Maths',
    years: '7–10',
    skill: 'Mathematical reasoning',
    minutes: 10,
    note: 'Put a worked example with an error on the board first. Needs a worked example containing an error, on the board.',
    prompts: [
      'Scan the working without calculating yet. Which line looks wrong?',
      'Locate the first incorrect step.',
      'What was the student trying to do there?',
      'Why does that step fail?',
      'Write the correct step.',
      'Finish the solution from that point.',
    ],
  }),
  set({
    id: 'bank-maths-28',
    name: 'Explain Your Method',
    subject: 'Maths',
    years: '7–10',
    skill: 'Mathematical reasoning',
    minutes: 10,
    note: 'Fire after students have finished a problem. Best as revise — needs a completed problem.',
    prompts: [
      'Name the strategy you used.',
      'Why that strategy — not another?',
      'Show only the steps that earn the answer.',
      'Which step demanded the most thinking?',
      'How did you check?',
      'Explain it so a classmate who was stuck could restart from your words.',
    ],
  }),
  set({
    id: 'bank-maths-29',
    name: 'Compare Two Strategies',
    subject: 'Maths',
    years: '7–10',
    skill: 'Mathematical reasoning',
    minutes: 10,
    note: 'Show Strategy A and B on the board, or have two student methods ready. Needs two methods on the board, or two student methods ready.',
    prompts: [
      'How does Strategy A actually work?',
      'How does Strategy B work?',
      'What do they share underneath?',
      'Where do they differ?',
      'Which is more efficient for this problem — and why?',
      'When would the other strategy be the smarter choice?',
    ],
  }),
  set({
    id: 'bank-maths-30',
    name: 'Interrogate the Data',
    subject: 'Maths',
    years: '7–10',
    skill: 'Data',
    minutes: 10,
    note: 'Needs a graph or table visible. Needs a graph or table visible.',
    prompts: [
      'What does this graph or table claim to represent?',
      'What pattern do you see — not what you expected to see?',
      'What comparison is most revealing?',
      'What looks unusual or suspicious?',
      'What calculation would unlock the story?',
      'What conclusion can you defend — and what can you not?',
    ],
  }),
  set({
    id: 'bank-maths-unstick-01',
    name: 'Get Started — Maths',
    subject: 'Maths',
    years: '5–12',
    skill: 'Unstick',
    minutes: 3,
    note: 'NEW. For a student who has read the problem three times and written nothing. Needs a problem in front of them.',
    prompts: [
      'Copy out the question in your own words.',
      'Write down every number the question gives you.',
      'What are you being asked to find? One thing.',
      'Name one thing you could try first — even if you are not sure it works.',
    ],
  }),

  // Science
  set({
    id: 'bank-sci-31',
    name: 'Predict–Observe–Explain',
    subject: 'Science',
    years: '5–8',
    skill: 'Scientific thinking',
    minutes: 10,
    note: 'Pause between predict / observe / explain during the demo. Needs a demonstration or experiment.',
    prompts: [
      'What do you predict will happen?',
      'Why — using science, not guesswork?',
      'What actually happened?',
      'Was your prediction supported, partly right, or wrong?',
      'What scientific idea best explains the result?',
    ],
  }),
  set({
    id: 'bank-sci-32',
    name: 'Nail the Variables',
    subject: 'Science',
    years: '7–8',
    skill: 'Investigations',
    minutes: 8,
    prompts: [
      'Write the investigation question in one line.',
      'What will you deliberately change?',
      'What will you measure?',
      'What must stay the same?',
      'Why would changing those "same" things ruin the test?',
      'How will you make this a fair test?',
    ],
  }),
  set({
    id: 'bank-sci-33',
    name: 'Plan an Investigation',
    subject: 'Science',
    years: '7–8',
    skill: 'Investigations',
    minutes: 12,
    prompts: [
      'What question are you actually testing?',
      {
        prompt: 'State a testable hypothesis.',
        helper: 'If… then… because…',
      },
      'Independent variable: what you change.',
      'Dependent variable: what you measure.',
      'Controlled variables: what you keep constant.',
      'What equipment do you need — and why each item?',
      'What data will convince you the hypothesis is supported or not?',
    ],
  }),
  set({
    id: 'bank-sci-34',
    name: 'Claim–Evidence–Reasoning',
    subject: 'Science',
    years: '9–10',
    skill: 'Scientific explanation',
    minutes: 10,
    prompts: [
      {
        prompt: 'C — Claim: What are you asserting?',
        helper: 'One clear sentence. No padding.',
      },
      'E — Evidence: Which data or observation supports it?',
      'R — Reasoning: Which scientific concept links evidence to claim?',
      'How exactly does the evidence force that claim?',
      'What evidence does not fit — and what do you do with it?',
      'Write a conclusion that a doubtful reader could still respect.',
    ],
  }),
  set({
    id: 'bank-sci-35',
    name: 'Read the Results',
    subject: 'Science',
    years: '7–10',
    skill: 'Data analysis',
    minutes: 10,
    note: 'Needs results, a graph or a table in view. Needs results, a graph or a table in view.',
    prompts: [
      'What trend or pattern is actually there?',
      'Which piece of evidence most strongly supports that reading?',
      'Which result looks like an outlier?',
      'How strong is the pattern — clear, weak, or are you hoping?',
      'What scientific explanation could account for it?',
      'What conclusion is fair — and what would be claiming too much?',
    ],
  }),
  set({
    id: 'bank-sci-36',
    name: 'Evaluate the Method',
    subject: 'Science',
    years: '9–10',
    skill: 'Investigations',
    minutes: 12,
    note: 'Fire after an investigation is finished and results are in. Best as revise — needs completed results from an investigation.',
    prompts: [
      'How reliable are these results?',
      'Which results look inconsistent — and why might that be?',
      'Name the most likely sources of error.',
      'Was the method actually suited to the question?',
      'What one change would improve the investigation most?',
      'What further test would make the conclusion harder to dismiss?',
    ],
  }),
  set({
    id: 'bank-sci-37',
    name: 'Build a Scientific Explanation',
    subject: 'Science',
    years: '7–10',
    skill: 'Scientific writing',
    minutes: 10,
    prompts: [
      'What phenomenon are you explaining?',
      'Which scientific ideas must appear in a good explanation?',
      'What evidence or example will you lean on?',
      'Connect the evidence to the science — show the chain.',
      'Find one everyday word in your answer and replace it with precise scientific language. Type: old word → new word.',
      'Finish: why does this happen?',
    ],
  }),
  set({
    id: 'bank-sci-38',
    name: 'Evaluate Scientific Evidence',
    subject: 'Science',
    years: '11–12',
    skill: 'Senior science',
    minutes: 15,
    prompts: [
      'What claim or conclusion is on the table?',
      'What evidence is offered for it?',
      {
        prompt: 'How reliable is that evidence?',
        helper: 'Sample, method, bias, reproducibility…',
      },
      'Where is the uncertainty, error or limitation?',
      'How valid is the leap from evidence to conclusion?',
      'What alternative explanations survive?',
      'What additional evidence would change your view?',
    ],
  }),
  set({
    id: 'bank-science-unstick-01',
    name: 'Get Started — Science',
    subject: 'Science',
    years: '5–12',
    skill: 'Unstick',
    minutes: 3,
    note: 'NEW. For a student stuck at the start of an explanation or investigation.',
    prompts: [
      'In one sentence, what are you trying to explain or find out?',
      'What do you already know about it? Anything counts.',
      'Name one scientific word that belongs in your answer.',
      'Write one sentence using that word. Rough is fine.',
    ],
  }),

  // General / cross-curricular
  set({
    id: 'bank-gen-39',
    name: 'Retrieval Warm-Up',
    subject: 'General',
    years: 'All',
    skill: 'Lesson opener',
    minutes: 8,
    prompts: [
      'Without looking: what stuck from last lesson?',
      'Write three important ideas — not three titles.',
      'Give one concrete example.',
      'Which idea are you least certain about?',
      'Write the one question you still need answered.',
    ],
  }),
  set({
    id: 'bank-gen-40',
    name: 'Confidence Pulse',
    subject: 'General',
    years: 'All',
    skill: 'Formative check',
    minutes: 5,
    prompts: [
      'Confidence right now: 1–5. Be honest.',
      'What part feels under control?',
      'Where are you unsure?',
      'What single thing would lift you one point?',
    ],
  }),
  set({
    id: 'bank-gen-41',
    name: 'Kill the Misconception',
    subject: 'General',
    years: 'All',
    skill: 'Understanding check',
    minutes: 8,
    note: 'Put a tempting wrong statement or example on the board. Needs a tempting wrong statement or example on the board.',
    prompts: [
      'Read it carefully. What seems wrong here?',
      'Name the incorrect idea.',
      'Why is it wrong?',
      'State the correct idea cleanly.',
      'What makes this mistake so easy to fall for?',
    ],
  }),
  set({
    id: 'bank-gen-42',
    name: 'Exit Ticket That Matters',
    subject: 'General',
    years: 'All',
    skill: 'Lesson close',
    minutes: 6,
    prompts: [
      'What is the one idea from today you must not forget?',
      'Explain it in your own words — no copying the slide.',
      'Give an example from today\'s work.',
      'What can you do now that you could not do at the start?',
      'What are you still unsure about?',
    ],
  }),
  set({
    id: 'bank-gen-43',
    name: 'Peer Feedback That Helps',
    subject: 'General',
    years: '7–12',
    skill: 'Feedback',
    minutes: 8,
    note: 'Students need a peer\'s work visible. Best as revise — needs a peer\'s work visible.',
    prompts: [
      'What is working well — point to a specific line or move?',
      'Quote or describe that strength.',
      'Where does the meaning become unclear?',
      'Ask the writer one sharp question.',
      'Give one change they could make in the next five minutes.',
    ],
  }),
  set({
    id: 'bank-gen-44',
    name: 'Use the Feedback',
    subject: 'General',
    years: '7–12',
    skill: 'Writing & feedback',
    minutes: 12,
    note: 'Perfect after Inbox notes or board comments. Best as revise — needs teacher or peer feedback on existing work.',
    prompts: [
      'Which section are you improving?',
      'Read the teacher comment again. What is it actually asking?',
      'In your words: what must change?',
      'Describe the revision you will make before you make it.',
      'Make the change. Paste the new version.',
      'How is the work stronger now?',
    ],
  }),
  set({
    id: 'bank-gen-45',
    name: 'Own the Key Word',
    subject: 'General',
    years: '5–10',
    skill: 'Vocabulary',
    minutes: 8,
    note: 'Put the target word on the board. Needs the target word on the board.',
    prompts: [
      'What do you think this word means in this subject?',
      'Say it in plain words a Year 5 student would understand.',
      'Use it in a sentence about today\'s work.',
      'Name a near-synonym — and how it differs.',
      'Give a clear example.',
      'What do people often think it means that it does not?',
    ],
  }),
  set({
    id: 'bank-gen-46',
    name: 'Compare to Decide',
    subject: 'General',
    years: '5–10',
    skill: 'Thinking routine',
    minutes: 8,
    prompts: [
      'What do these two things share?',
      'Name one difference that actually matters.',
      'Find a second difference.',
      'Which difference changes your judgment most?',
      'Why that one?',
      'What conclusion does the comparison lead you to?',
    ],
  }),
  set({
    id: 'bank-gen-47',
    name: 'Live Work Check-In',
    subject: 'General',
    years: 'All',
    skill: 'Live monitoring',
    minutes: 5,
    note: 'Send mid-task when you need eyes on the room fast.',
    prompts: [
      'What are you doing right now — in one sentence?',
      'What have you finished?',
      'What is next?',
      {
        prompt: 'Where are you stuck, if anywhere?',
        helper: 'If stuck: name the exact step that broke.',
      },
      'What do you need from your teacher in the next two minutes?',
    ],
  }),
  set({
    id: 'bank-gen-48',
    name: 'Teach It Back',
    subject: 'General',
    years: 'All',
    skill: 'Understanding',
    minutes: 8,
    prompts: [
      'Explain today\'s idea to someone who missed the lesson.',
      'What must they understand first?',
      'Give the example that makes it click.',
      'What mistake should they avoid?',
      'What question would prove they got it?',
    ],
  }),
  set({
    id: 'bank-gen-unstick-01',
    name: 'I Don\'t Know Where to Start',
    subject: 'General',
    years: 'All',
    skill: 'Unstick',
    minutes: 3,
    note: 'NEW. Subject-neutral. Send when a student says "I don\'t get it" and you need to find out where the block actually is.',
    prompts: [
      'In your own words, what is the task asking you to do?',
      'What part do you understand already?',
      'What is the exact bit that stopped you?',
      'Write down anything at all you could put on the page right now.',
    ],
  }),
  set({
    id: 'bank-gen-11',
    name: 'Unpack the Command Word',
    subject: 'General',
    years: '7–12',
    skill: 'Task literacy',
    minutes: 6,
    note: 'Fire before any extended response. Students lose marks for describing when the task said evaluate. Needs the question or task in front of them.',
    prompts: [
      {
        prompt: 'Copy out the command word in the question.',
        helper: 'Explain, evaluate, analyse, compare, justify, discuss…',
      },
      'In your own words, what is that word asking you to actually do?',
      {
        prompt: 'What would a weak answer do instead?',
        helper: 'Usually: retell, describe, or list.',
      },
      'How many parts does this question have? List them.',
      'Write the first sentence of your answer so it does what the command word asks.',
    ],
  }),
  set({
    id: 'bank-gen-12',
    name: 'Read the Rubric',
    subject: 'General',
    years: '7–12',
    skill: 'Assessment',
    minutes: 8,
    note: 'Students are marked against criteria they have often never read. Works cold or on a draft — needs the marking rubric or criteria sheet.',
    prompts: [
      'Find the band you are aiming for. Copy the words that describe it.',
      'Which word in that band is the hardest to prove?',
      'What would that look like in your work?',
      'Which of those words does your draft not yet earn?',
      'Name the one change that would move you up a band.',
    ],
  }),
  set({
    id: 'bank-gen-13',
    name: 'Self-Check Before You Submit',
    subject: 'General',
    years: '7–12',
    skill: 'Assessment',
    minutes: 8,
    note: 'Last-look set. Catches the marks students throw away. Best as revise — needs a finished or near-finished draft.',
    prompts: [
      'Reread the task. Quote the exact words of what it asked for.',
      'Which sentence of yours answers that most directly?',
      'What has the task asked for that you have not done?',
      'Check the word count and format. What is off?',
      'Fix the biggest gap now. Paste what you changed.',
    ],
  }),
  set({
    id: 'bank-gen-14',
    name: 'Catch Up — You Missed This',
    subject: 'General',
    years: 'All',
    skill: 'Classroom routines',
    minutes: 5,
    note: 'Send to a returning student while you teach the rest of the class. Tells you the size of the gap in 60 seconds.',
    prompts: [
      'What was the last lesson you were here for?',
      'What do you remember covering that day?',
      'Look at what the class is doing now. What part makes no sense to you?',
      'What do you think you need to catch up on first?',
      'What do you need from me to get moving in the next five minutes?',
    ],
  }),
  set({
    id: 'bank-gen-15',
    name: 'Cover Lesson Starter',
    subject: 'General',
    years: 'All',
    skill: 'Classroom routines',
    minutes: 6,
    note: 'For a relief teacher with an unfamiliar class. Gets thirty students typing in four minutes and tells you what the class thinks the plan is.',
    prompts: [
      'What are you supposed to be working on today?',
      'Where are you up to?',
      'What did your teacher tell you to do if you finish?',
      'What is the one thing you need help with this lesson?',
      'Write the first line of the work you are about to do.',
    ],
  }),
  set({
    id: 'bank-gen-16',
    name: 'Group Work — Who\'s Doing What',
    subject: 'General',
    years: '5–12',
    skill: 'Collaboration',
    minutes: 6,
    note: 'Every member answers separately. Mismatched answers show you which groups have a problem before it becomes one. Needs students in groups.',
    prompts: [
      'What is your group making or deciding?',
      'What are you personally responsible for?',
      'What is each other member doing?',
      'What needs to be finished by the end of this lesson?',
      'What is slowing your group down right now?',
    ],
  }),
  set({
    id: 'bank-gen-17',
    name: 'Group Work — Check the Progress',
    subject: 'General',
    years: '5–12',
    skill: 'Collaboration',
    minutes: 5,
    note: 'Mid-project. Send to every member individually. Best as revise — needs group work already underway.',
    prompts: [
      'What have you finished since last lesson?',
      'What is still not done?',
      'What has changed about the plan?',
      'What do you need from another group member?',
      'What is the next thing you will do, in the next ten minutes?',
    ],
  }),
  set({
    id: 'bank-gen-18',
    name: 'Exam Prep Audit',
    subject: 'General',
    years: '9–12',
    skill: 'Revision',
    minutes: 8,
    note: 'Run a fortnight out. Tells you what to reteach.',
    prompts: [
      'List the topics on this exam.',
      'Which topic would you least like to see on the paper?',
      'Why that one — is it the content, or the question type?',
      'Which topic could you teach to someone else right now?',
      {
        prompt: 'What will you revise first, and how?',
        helper: 'Rereading notes is not a method.',
      },
      'What do you need from me before the exam?',
    ],
  }),
  set({
    id: 'bank-gen-19',
    name: 'Post-Assessment Reflection',
    subject: 'General',
    years: '7–12',
    skill: 'Assessment',
    minutes: 8,
    note: 'Run when handing back. Moves students past the number. Best as revise — needs returned marked work.',
    prompts: [
      'Which question or section went best?',
      'Which mark surprised you, and why?',
      'Find one error. What kind was it — knowledge, misreading, or running out of time?',
      'Read the feedback. What is it asking you to change?',
      'What will you do differently next time? Be specific.',
    ],
  }),
  set({
    id: 'bank-gen-20',
    name: 'Is This Source Any Good?',
    subject: 'General',
    years: '7–12',
    skill: 'Research',
    minutes: 8,
    note: 'Online research, not historical sources. Different skill to IOPCAM. Needs a website or article students are using.',
    prompts: [
      'Who published this? Copy the name.',
      'When was it written or last updated?',
      'What are they trying to get you to think, buy or do?',
      'Where does their information come from?',
      'Find one claim you could check somewhere else. Quote it.',
      'Would you use this in your work? Give the reason.',
    ],
  }),
  set({
    id: 'bank-gen-21',
    name: 'Build a Research Question',
    subject: 'General',
    years: '9–12',
    skill: 'Research',
    minutes: 10,
    prompts: [
      'What topic are you interested in?',
      'What do you already know about it?',
      'Turn that topic into a question starting with how, why or to what extent.',
      'Is your question too big to answer? Rewrite it narrower.',
      'What kind of evidence would answer it?',
      'What would a surprising answer look like?',
    ],
  }),
  set({
    id: 'bank-gen-22',
    name: 'Notes from Video or Documentary',
    subject: 'General',
    years: '5–12',
    skill: 'Note-taking',
    minutes: 8,
    note: 'Send before you press play. Pause at the halfway mark to collect. Needs a video playing or just finished.',
    prompts: [
      'What is this video about, in one sentence?',
      'Write down three facts you did not know.',
      'What is the one moment or image you will remember?',
      'What point is the video trying to make?',
      'What question did it leave you with?',
    ],
  }),
  set({
    id: 'bank-gen-25',
    name: 'Set a Goal That Sticks',
    subject: 'General',
    years: '5–12',
    skill: 'Reflection',
    minutes: 8,
    note: 'Start of term or after reports.',
    prompts: [
      'What went well for you last term?',
      'What is one thing you want to be better at by the end of this term?',
      'How would you know you had improved? What would be different?',
      'What is one thing you will do every week to get there?',
      'What usually stops you?',
      'What will you do when that happens?',
    ],
  }),
  set({
    id: 'bank-gen-26',
    name: 'Before Your Parent Interview',
    subject: 'General',
    years: '5–12',
    skill: 'Reporting',
    minutes: 8,
    note: 'Collects student voice before interviews. Gives you something specific to open with.',
    prompts: [
      'What is one piece of work this term you are proud of?',
      'Why that one?',
      'What has been hardest?',
      'What would you like your parents to know that they might not?',
      'What is one thing you want help with next term?',
    ],
  }),

];

const YEAR_ORDER = ['5–6', '7–8', '9–10', '11–12'];

function expandYears(label) {
  const raw = String(label || '').replace(/-/g, '–').trim();
  if (!raw || raw === 'All') return new Set(YEAR_ORDER);
  if (YEAR_ORDER.includes(raw)) return new Set([raw]);
  const match = raw.match(/^(\d+)–(\d+)$/);
  if (!match) return new Set(YEAR_ORDER);
  const start = Number(match[1]);
  const end = Number(match[2]);
  const covered = new Set();
  for (const band of YEAR_ORDER) {
    const [a, b] = band.split('–').map(Number);
    if (a <= end && b >= start) covered.add(band);
  }
  return covered.size ? covered : new Set(YEAR_ORDER);
}

export function setMatchesFilters(set, subject, yearBand) {
  if (subject && subject !== 'All' && set.subject !== subject) return false;
  if (!yearBand || yearBand === 'All') return true;
  return expandYears(set.years).has(yearBand);
}

export function formatSetMeta(set) {
  const years = set.years === 'All' ? 'All years' : `Years ${set.years}`;
  const mins = set.minutes ? `~${set.minutes} min` : '';
  return [set.subject, years, set.skill, mins].filter(Boolean).join(' · ');
}
