...
<body>
  <h1>Participation report · Room ${escapeHtml(report.roomCode)}</h1>
...
export function downloadLessonReportHtml(report) {
  ...
  downloadTextFile(`iboard-participation-${report.roomCode}-${stamp}.html`, buildLessonReportHtml(report), 'text/html;charset=utf-8');
}

export function downloadLessonReportCsv(report) {
  const stamp = stampForFilename(new Date(report.generatedAt || Date.now()));
  downloadTextFile(`iboard-participation-${report.roomCode}-${stamp}.csv`, buildLessonReportCsv(report), 'text/csv;charset=utf-8');
}
