...
export default function LessonReportPanel({ roomCode, onClose, embedded = false }) {
    ...
    <>
      {!embedded ? (
        <div className="flex shrink-0 items-start justify-between gap-3 border-b border-slate-200 px-5 py-4 dark:border-slate-700">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.16em] text-indigo-600 dark:text-indigo-300">Participation</p>
      ...
      <nav aria-label="Report views" className={`flex shrink-0 gap-1 ${embedded ? 'mb-3 border-b border-slate-200 pb-0 dark:border-slate-700' : 'border-b border-slate-200 bg-slate-50 px-4 pt-2 dark:border-slate-700 dark:bg-slate-950/50'}`}>
        {[
          ['students', 'By student'],
      ...
      <div className={embedded ? 'min-h-0' : 'min-h-0 flex-1 overflow-y-auto p-4 scrollbar-thin'}>
