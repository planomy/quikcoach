        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {visible.map((response) => (
            <article key={response.studentId} className="rounded-2xl border border-indigo-200 bg-indigo-50 p-4 text-indigo-950 shadow-sm dark:border-indigo-800 dark:bg-indigo-950 dark:text-indigo-100">
              <p className={`${display ? 'text-xl' : 'text-sm'} leading-relaxed`}>“{formatLiveAnswer(response.value)}”</p>
              {!display && (
...
export default function LiveResponseTeacher({
  ...
  function nudge(studentId) {
    socket.emit('teacher:live-nudge', { studentId }, (ack) => {
      setMessage(ack?.ok ? 'Check-in sent — waiting for their status.' : 'Student is not available.');
    });
  }
  ...
  function improveFeatured(item) { launch({ type: 'short', prompt: `Improve this answer: “${formatLiveAnswer(item.value)}”`, options: [], correctAnswer: '', anonymous: false, optional: false, imageUrl: '', timerSeconds: 0 }); }
  ...
  function askComparison() {
    ...
    launch({ type: 'choice', prompt: `Which response is stronger? A: “${formatLiveAnswer(chosen[0].value)}” B: “${formatLiveAnswer(chosen[1].value)}”`, options: ['Answer A', 'Answer B', 'Both are effective'], correctAnswer: '', anonymous: false, optional: false, imageUrl: '', timerSeconds: 0 });
    setWallMode('');
  }
      ...
      {embedded && (
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 px-3 py-2 dark:border-slate-800">
          ...
          {typeof onExpand === 'function' && (
            <CloseButton onClick={() => onExpand(false)} label="Hide panel" />
          )}
        </div>
      )}
