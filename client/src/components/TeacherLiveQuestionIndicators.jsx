 ...
 * has a waiting audience question. Clicking the badge keeps the teacher in LIVE
 * and opens a compact floating question card with the common actions.
 */
export default function TeacherLiveQuestionIndicators() {
  ...
  function updateQuestion(action, anonymous) {
    ...
    connection.socket.emit(
      'teacher:qna-status',
      { questionId: selectedQuestion.id, action, anonymous },
      (ack) => {
  ...
  function askClass() {
    ...
    connection.socket.emit(
      'teacher:qna-ask-room',
      {
        ...
        anonymous: !!selectedQuestion.anonymousRequested,
      },
      (ack) => {
        if (!ack?.ok) {
  ...
  const selectedPosition = selectedQuestion
    ? queuePositionById.get(Number(selectedQuestion.id))
    : null;
      ...
      {selectedQuestion && selectedPosition && (
        ...
        >
          <div className="flex items-start gap-3">
            <span className="grid h-9 min-w-9 shrink-0 place-items-center rounded-full bg-fuchsia-600 px-1 text-sm font-black text-white">
              {selectedPosition}
            </span>
            <div className="min-w-0 flex-1">
              <div className="flex items-start gap-2">
                <p className="min-w-0 flex-1 truncate text-[10px] font-black uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">
                ...
                <button
                  ...
                  className="grid h-7 w-7 shrink-0 place-items-center rounded-full text-lg font-black text-slate-400 hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-800 dark:hover:text-slate-200"
              ...
              <p className="mt-1 text-base font-bold leading-snug text-slate-950 dark:text-white">
          ...
          {message && (
            <p className="mt-3 rounded-lg bg-red-50 px-3 py-2 text-xs font-bold text-red-700 dark:bg-red-950/40 dark:text-red-200">
              {message}
            </p>
          )}
          ...
          <div className="mt-4 flex flex-wrap items-start gap-2">
            <details>
              <summary className="list-none cursor-pointer rounded-lg bg-indigo-50 px-3 py-2 text-xs font-black text-indigo-700 dark:bg-indigo-950 dark:text-indigo-200">
                Share ▾
              </summary>
              <div className="mt-1 min-w-36 overflow-hidden rounded-lg border border-slate-200 bg-white p-1 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                <button
                  ...
                  onClick={(event) => {
                    ...
                    updateQuestion('publish', false);
                  }}
                  className="block w-full rounded-md px-3 py-2 text-left text-xs font-black text-indigo-700 hover:bg-indigo-50 dark:text-indigo-200 dark:hover:bg-indigo-950"
                >
                  Named
                </button>
                <button
                  ...
                  onClick={(event) => {
                    event.currentTarget.closest('details')?.removeAttribute('open');
                    updateQuestion('publish', true);
                  }}
                  className="block w-full rounded-md px-3 py-2 text-left text-xs font-black text-fuchsia-700 hover:bg-fuchsia-50 dark:text-fuchsia-200 dark:hover:bg-fuchsia-950"
                >
                  Anonymous
                </button>
              </div>
            ...
            <button
              type="button"
              onClick={askClass}
              className="rounded-lg bg-emerald-500 px-3 py-2 text-xs font-black text-emerald-950"
            >
              Ask class
            </button>
            <button
              type="button"
              onClick={() => updateQuestion('dismiss')}
              className="ml-auto rounded-lg bg-slate-100 px-3 py-2 text-xs font-black text-slate-600 hover:bg-red-50 hover:text-red-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-red-950/40 dark:hover:text-red-200"
            >
              Dismiss
            </button>
          </div>
